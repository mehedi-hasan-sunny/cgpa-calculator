<template>
    <v-card class="border-rad-0" color="#fafafa" flat height="100vh">
        <v-toolbar>
        ...
        <v-container>
            <v-data-table :headers="headers" :items="storedList" class="elevation-1 border-rad" hide-actions
                          item-key="name">
                <template v-slot:items="props">
                    <td @click="props.expanded = !props.expanded, subItems = props.item.data, expanded = !expanded"
                    ...
                    <td @click="props.expanded = !props.expanded, subItems = props.item.data, expanded = !expanded"
                        class="text-xs-center">
                        {{storedListSingleCGPA(props.item.data)}}
                    </td>
                ...
                <template slot="expand">
                    <GradePointList :dataArray="subItems" :headers="subheader" @deleteItem="deleteSubItem"
                                    @editItem="editSubItem"></GradePointList>
                </template>
            </v-data-table>
            <!--<v-card v-else>-->
            <!--<v-card-text>-->
        ...
        <v-dialog max-width="500px" v-model="dialog">
            <v-card>
                <v-card-text>
                    ...
                    <p class="text-xs-center">You want to delete this permanently?</p>
                </v-card-text>
...
<script>
    ...
    export default {
        ...
        watch: {
            dialog(val) {
                if (!val) this.dialogIsOpenedFor = null;
            }
        }
